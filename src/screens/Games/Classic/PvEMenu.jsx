...
export default function PvEMenu({navigation}) {
  return (
    <View style={style.container}>
        ...
        <Text style = {style.button} onPress = {() => {navigation.navigate("Classic_Game", {diff: "LOCAL"});console.log("Local");}}>
            Local PvP
            <Text style={style.description}>{'\n'}A local game between two players on your own phone</Text>
        </Text>
        <Text style = {style.button} onPress = {() => {navigation.navigate("Classic_Game", {diff: "BOT_EASY"})}}>
            Easy Bot
            <Text style={style.description}>{'\n'}A local game against a bot which is easy to defeat</Text>
        </Text>

        <Text style = {style.button} onPress = {() => {navigation.navigate("Classic_Game", {diff: "BOT_MEDIUM"})}}>
            Intermediate Bot
